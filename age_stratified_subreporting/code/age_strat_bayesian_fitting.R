...
age_stratified_bayesian_model <- function (casesKnownStrat,
  ...
  # DH: Distribution over slopes and intercepts of age dependent ouctome rate
  ...
  # DH: get vector with the estimated proportion of outcome for each
  # age bin (the columns in 'data')
  ageBins <- colnames(data)
  meanAges <- mid_bin_age(ageBins)
  probOutcomeStrat <- exp(meanAges * outcomeSlope + outcomeIntercept)/100

  # DH: Compute the expected number of outcomes at each timepoint. The
  # matrix 'data' has one column for each age bin, and each row is a time t.
  # Each row contains the number of cases from previous days that are
  # expected to resolve on the day t (that is, the cases from previous days
  # weighted by the symptoms to outcome delay distrubition).
  # This line multiplies the number of known cases expected to resolve
  # on the day for each age bin, by the estimated "real" outcome rate
  # for that age bin (probOutcomeStrat) and divides it by the unkown
  # reporting rate, which is what we are estimating. This will be
  # equal to the number of observed outcomes, given that we take reportin_rate
  # into account
